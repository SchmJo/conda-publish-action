...
upload_package(){
    ...
    if [[ $INPUT_PLATFORMS == *"osx"* ]]; then
    anaconda upload --label main osx-64/*.tar.bz2 --channel conda-forge
    fi
    if [[ $INPUT_PLATFORMS == *"linux"* ]]; then
    anaconda upload --label main linux-64/*.tar.bz2 --channel conda-forge
    fi
    if [[ $INPUT_PLATFORMS == *"win"* ]]; then
    anaconda upload --label main win-64/*.tar.bz2 --channel conda-forge
    fi
}
